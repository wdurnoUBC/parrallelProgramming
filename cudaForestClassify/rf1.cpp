...
// TODO Inifinite recursion is techinically possible--implement information gain to correct
...
void serializeForestHelper( node* tree , int thisCuNode , cuNode** out , int* length ) 
{
	...
	{
		(*out)[ thisCuNode ].left = serializeForestHelperAddNode( tree->left , out , length ) ; 
printf( "DEBUG, recieved %i\n" , (*out)[thisCuNode].left ) ; 
...
printf( "DEBUG, recieved %i\n" , (*out)[thisCuNode].right ) ; 
		serializeForestHelper( tree->right , (*out)[thisCuNode].right , out , length ) ; 
	}
	if( (*out)[thisCuNode].right == -2 ) // DEBUG !!! 
		printf( "Failed right detected\n" ) ; 
	if( (*out)[thisCuNode].left == -2 ) // DEBUG !!! 
                printf( "Failed left detected\n" ) ;
}
